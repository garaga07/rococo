...
Wiremock отдаст тот же набор данных, что и до удаления.
  ...
  И самое главное - у тебя есть проект rococo, который будет выступать образцом для подражания в разработке микросервисов.
Тестовое покрытие rococo, которого мы с тобой добились на настоящий момент, однако, является достаточно слабым - учтите это при написании тестов на Rococo - это,
...
твоя задача взять сервис rococo-auth и аккуратно переделать его для работы с rococo.
...
- register.html
  ...
  Основная задача - аккуратно заменить упоминания о rococo в этом сервисе, а в идеале - еще и
...
задеплоить свой сервис на прод, прикрутить CI/CD, без этого никак не обойдется.
  ...
  Я советую использовать плагин jib - как в rococo, для бэкендов, и самописный dockerfile для фронта.
Фронтенд использует фреймворк Svelte, но докеризация там работает ровно так же, как и для React в Rococo.
...
На самом деле, концептуально и технически каждый сервис будет похож на что-то из rococo, поэтому
главное внимательность и аккуратность. Любые отхождения от rococo возможны - ты можешь захотеть
...
###### Сервис gateway, работа с пагинацией
   ...
   В отличие от Rococo, проект Rococo использует пагинацию (что бы грузить данные с бэка по частям), а это значит что в `rococo-gateway` должны быть реализованы 
Pageble контроллеры.
Пример:
```java
  @GetMapping()
